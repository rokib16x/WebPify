...
function App() {
  ...
  const processImages = async (imagesToProcess) => {
    ...
    for (let i = 0; i < imagesToProcess.length; i++) {
      ...
      setImages((prev) => prev.map((img) => 
        img.id === currentImage.id ? { ...img, status: "processing", progress: 0 } : img
      ));
      ...
      try {
        ...
        setImages((prev) => prev.map((img) => 
          img.id === currentImage.id ? { ...img, status: "error", progress: 0 } : img
        ));
      }
    }
  ...
  const handleConvertAll = async () => {
    const imagesToProcess = images.filter(img => img.status !== "done" || optionsChanged);
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                <button
                  className="flex-1 py-3 px-4 bg-[#0267ff] hover:bg-[#0255ff] disabled:bg-[#f3f3f7] text-white font-semibold rounded-lg transition-colors shadow-sm disabled:cursor-not-allowed"
                  onClick={handleConvertAll}
                  disabled={isProcessing || images.length === 0}
                >
                  {isProcessing ? "Converting..." : optionsChanged ? "Apply New Settings" : "Convert to WebP"}
                </button>
              ...
              {isProcessing && (
                <div className="bg-white rounded-xl p-4 shadow-sm">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-[#2c2d2a]">Overall Progress</span>
                    <span className="text-sm font-medium text-[#0267ff]">{overallProgress}%</span>
                  </div>
              ...
              {optionsChanged && !isProcessing && images.some(img => img.status === "done") && (
                <div className="bg-[#fff8e6] border border-[#ffeeba] rounded-xl p-3 text-[#856404] text-sm">
                  <p>Compression settings have changed. Click "Apply New Settings" to update all images.</p>
                </div>
              )}
            </div>
          </div>
